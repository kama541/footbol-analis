            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex flex-col items-end gap-2">
                {device.status === 'Online' ? (
                  <span className="flex items-center gap-1.5 text-xs font-bold text-[#FFE600] bg-[#FFE600]/10 px-2 py-1 rounded">
                    <Wifi size={14} /> Online
                  </span>
                ) : (
            ...
            <div className="mt-auto grid grid-cols-2 gap-4 pt-4 border-t border-slate-800">
              <div>
                ...
                <div className="flex items-center gap-2 font-semibold text-slate-300">
                  {device.battery !== null ? (
                    <>
                      <Battery size={16} className={device.battery < 20 ? 'text-red-500' : 'text-[#FFE600]'} />
                      {device.battery}%
                    </>
                  ) : (
              ...
              <div>
                ...
                <div className="flex items-center gap-2 font-semibold text-slate-300">
                  <RefreshCw size={14} className="text-[#FFE600]" />
                  {device.lastSync}
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-[400px] shadow-2xl animate-fade-in border border-slate-200 dark:border-slate-800">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-white dark:text-white">Register Device</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  {[1,2,3,4,5,6].map(i => (
                    <input key={i} type="text" maxLength={1} className="w-12 h-12 text-center text-xl font-bold border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white outline-none focus:border-[#FFE600]" />
                  ))}
                </div>
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button onClick={() => setShowAddDevice(false)} className="flex-1 py-2 bg-[#FFE600] text-black rounded-lg font-bold hover:bg-yellow-500 transition-colors">
                Connect
              </button>
            </div>
          </div>
