  ...
  return (
    <div className="p-8 h-full overflow-y-auto custom-scrollbar relative z-0">
      ...
      <div className="flex items-center justify-between mb-10">
        <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight drop-shadow-sm">Playlists</h1>
        <button className="glass-button flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-football-blue to-football-purple text-white rounded-xl font-bold shadow-lg shadow-football-blue/25 hover:shadow-football-blue/40 text-sm tracking-wide">
          <Plus size={18} />
          Create Playlist
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {playlists.map((pl, i) => (
          <div key={i} onClick={() => navigate(`/playlists/${pl.id}`)} className="glass-card rounded-3xl overflow-hidden group cursor-pointer border border-slate-200/50 dark:border-slate-700/50">
            <div className={`h-40 ${pl.color} relative overflow-hidden flex items-center justify-center transition-all duration-500`}>
              <div className="absolute inset-0 bg-gradient-to-b from-black/5 to-black/30 dark:from-black/20 dark:to-black/60 group-hover:opacity-80 transition-opacity"></div>
            ...
            <div className="p-6 relative">
              <div className="absolute top-0 right-6 -translate-y-1/2 w-10 h-10 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center shadow-lg border border-slate-100 dark:border-slate-700">
                <span className={`w-3 h-3 rounded-full ${pl.color} shadow-sm animate-pulse`}></span>
              </div>
              <h3 className="text-lg font-black text-slate-900 dark:text-white mb-2 group-hover:text-football-blue transition-colors">{pl.title}</h3>
              <div className="flex items-center gap-3 text-sm text-slate-500 dark:text-slate-400 font-semibold tracking-wide">
