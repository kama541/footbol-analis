  ...
  return (
    <div className="flex flex-col h-full bg-[#161920] backdrop-blur-xl relative z-10 overflow-hidden shadow-[-10px_0_20px_rgba(0,0,0,0.05)] border-l border-slate-800">
      <div className="p-4 border-b border-slate-800 bg-[#161920] backdrop-blur-md sticky top-0 z-20">
        <h2 className="font-black text-white tracking-tight text-lg">{t('events.recent_events')}</h2>
      </div>
        ...
        {events.length === 0 ? (
          ...
          events.map((event, index) => (
            <div 
              ...
              className="glass-card bg-slate-800/60 border border-slate-700/50 rounded-2xl p-4 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 cursor-pointer group animate-slide-up relative overflow-hidden"
              onClick={() => {
            ...
            >
              ...
              <div className="flex justify-between items-start mb-3 relative z-10">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-[10px] font-black text-slate-400 bg-slate-700/50 px-2 py-0.5 rounded-md">
                ...
                <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button 
                    className="p-1.5 text-slate-400 hover:text-[#FFE600] hover:bg-slate-700 rounded-lg transition-colors shadow-sm bg-slate-800/50 backdrop-blur-sm"
              ...
              <div className="relative z-10">
                <p className="text-sm font-black text-white group-hover:text-[#FFE600] transition-colors">{event.player}</p>
                <p className="text-xs text-slate-400 mt-1 font-medium">{event.description}</p>
              </div>
            </div>
          ))
        )}
